...
class EmbeddingService {
  ...
  static async generateAndStoreEmbeddingForCourse(courseId) {
    ...
    if (!text.trim()) {
      throw new AppError('No content to embed', 400);
    }
  ...
  static async generateAndStoreEmbeddingForBlog(blogId) {
    ...
    if (!blog.published) {
      throw new AppError('Can only embed published blogs', 400);
    }
  ...
  static async storeVectorEmbedding(embeddingId, vector) {
    // Store vector in MongoDB as array of numbers
    // In production, consider using pgvector, Pinecone, or Weaviate
  ...
  static async deleteEmbedding(embeddingId) {
    const embedding = await Embedding.findByIdAndDelete(embeddingId);
    return embedding;
  }
}
