...
class CommentService {
  static async addComment(userId, blogId, content, parentCommentId = null) {
    ...
    if (!blog || !blog.published) throw new AppError('Blog not found or not published', 404);
    if (parentCommentId) {
    ...
    await Blog.findByIdAndUpdate(blogId, { $inc: { 'stats.commentsCount': 1 } });
  ...
  static async listComments(blogId, query = {}) {
    ...
    const filter = { targetType: 'blog', targetId: blogId, parent: null, status: 'visible' };
  ...
  static async likeComment(commentId) {
    const comment = await Comment.findByIdAndUpdate(commentId, { $inc: { votes: 1 } }, { new: true })
      .populate('author', 'fullName publicUsername avatarUrl');
  ...
  static async deleteComment(commentId, userId) {
    ...
    if (!comment) throw new AppError('Comment not found', 404);
    if (comment.author.toString() !== userId) throw new AppError('Not authorized to delete this comment', 403);
