...
class BlogService {
  ...
  static async createBlog(userId, data) {
    ...
    if (existingBlog) throw new AppError('Blog slug already exists', 400);
  ...
  static async publishBlog(blogId, userId) {
    ...
    if (blog.author.toString() !== userId) throw new AppError('Not authorized to publish this blog', 403);
    if (blog.published) throw new AppError('Blog is already published', 400);
    blog.published = true;
  ...
  static async unpublishBlog(blogId, userId) {
    const blog = await Blog.findById(blogId);
    if (!blog) throw new AppError('Blog not found', 404);
    if (blog.author.toString() !== userId) throw new AppError('Not authorized to unpublish this blog', 403);
    if (!blog.published) throw new AppError('Blog is not published', 400);
  ...
  static async getBlogBySlug(slug) {
    const blog = await Blog.findOne({ slug, published: true }).populate('author', 'fullName publicUsername avatarUrl');
    if (!blog) throw new AppError('Blog not found', 404);
    return blog;
  }
  ...
  static async getBlogById(blogId, userId = null) {
    ...
    if (!blog) throw new AppError('Blog not found', 404);
    if (!blog.published && (!userId || blog.author._id.toString() !== userId)) throw new AppError('Not authorized to view this blog', 403);
    return blog;
  }
  ...
  static async updateBlog(blogId, userId, data) {
    const blog = await Blog.findById(blogId);
    if (!blog) throw new AppError('Blog not found', 404);
    if (blog.author.toString() !== userId) throw new AppError('Not authorized to update this blog', 403);
    if (blog.published) throw new AppError('Cannot update published blog', 400);
    const allowedFields = ['title', 'content', 'tags'];
    const updates = {};
    allowedFields.forEach((field) => { if (data[field] !== undefined) updates[field] = data[field]; });
    if (updates.title) updates.slug = this.generateSlug(updates.title);
  ...
  static async deleteBlog(blogId, userId) {
    const blog = await Blog.findById(blogId);
    if (!blog) throw new AppError('Blog not found', 404);
    if (blog.author.toString() !== userId) throw new AppError('Not authorized to delete this blog', 403);
  ...
  static async getAuthorBlogs(userId, query = {}) {
    ...
    const limit = Math.min(50, parseInt(query.limit) || 10);
    const skip = (page - 1) * limit;
    const blogs = await Blog.find({ author: userId }).skip(skip).limit(limit).sort({ createdAt: -1 }).lean();
